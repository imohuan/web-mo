...
export function useCodeEditor() {
  const currentCode = ref(`<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="UTF-8">
...
<body>
  ...
  const history = ref<HistoryItem[]>([{
    title: generateHistoryTitle(currentCode.value) + "啊但是发动机和开发的快乐哈德良开发啊开朗大方好看啦活动是快乐发的但是",
  ...
  const monitorStreamContent = (content: string) => {
    ...
    if (isInCodeBlock.value) {
      ...
      if (detectionBuffer.value.includes('```')) {
        ...
        const contentToAdd = newContent.substring(0, newContent.length - (detectionBuffer.value.length - endIndex - 3))
  ...
  const handleCompleteCodeBlock = (fullCodeBlock: string) => {
    ...
    if (codeBlockMatch) {
      ...
      if (!detectAndApplyDiff(codeContent)) {
        // 如果不是差异格式，按原有逻辑处理
        if (language === 'html' || language === '' || codeContent.includes('<!DOCTYPE') || codeContent.includes('<html')) {
...
${extractedHtml}
  ...
  const updateCssInHtml = (cssContent: string) => {
    ...
    if (styleRegex.test(html)) {
      ...
      if (headRegex.test(html)) {
        html = html.replace(headRegex, '    <style>\\n' + cssContent + '\\n    </style>\\n</head>')
      }
    }
  ...
  const updateJsInHtml = (jsContent: string) => {
    ...
    if (scriptRegex.test(html)) {
      ...
      if (html.includes(bodyEndTag)) {
        html = html.replace(bodyEndTag, '    <script>\\n' + jsContent + '\\n    </script>\\n' + bodyEndTag)
      }
    }
  ...
  const handleUndo = () => {
    if (canUndo.value) {
      currentHistoryIndex.value--
      currentCode.value = history.value[currentHistoryIndex.value].code
    }
  }
  ...
  detectAndApplyDiff(`------- SEARCH
            class="bg-blue-50 bg-opacity-70 border border-blue-200 rounded-lg p-4 mb-4 shadow-sm hover:shadow-md transition-all duration-200 ease-in-out transform hover:-translate-y-1"
          >
            <h2 class="text-xl font-semibold text-blue-800 mb-1">{{ item.title }}</h2>
            <p class="text-gray-700 text-sm">{{ item.description }}</p>
            <a :href="item.link" target="_blank" class="text-blue-600 hover:text-blue-800 text-sm mt-2 inline-block"
=======
            class="bg-teal-50 border border-teal-200 rounded-lg p-4 mb-4 shadow-sm hover:shadow-md transition-all duration-200 ease-in-out transform hover:-translate-y-1"
          >
            <h2 class="text-xl font-semibold text-teal-800 mb-1">{{ item.title }}</h2>
            <p class="text-gray-700 text-sm">{{ item.description }}</p>
            <a :href="item.link" target="_blank" class="text-teal-600 hover:text-teal-800 text-sm mt-2 inline-block"
++++++++REPLACE`)
  return {
    currentCode,
    isPreviewMode,
